 ...
 * Template Post Type: practice_area
 *
 * @package CustomTheme
 */
...
<main id="main" class="site-main">
  ...
  if ( have_posts() ) :
    while ( have_posts() ) :
      ...
      $related_information_groups = function_exists( 'get_field' ) ? get_field( 'related_information_groups' ) : array();
      if ( ! is_array( $related_information_groups ) || empty( $related_information_groups ) ) {
        // ACF's repeater 'default_value' only pre-fills the admin form for a
        // never-saved field — it doesn't affect get_field() reads. Fall back
        // here so an entry with no Related Information saved yet still shows
        // the default list on the front end.
        $related_information_groups = function_exists( 'custom_theme_practice_area_default_related_information_groups' )
          ? custom_theme_practice_area_default_related_information_groups()
          : array();
      }
      ?>

      <section class="practice-area-hero">
        <div class="practice-area-hero__container">

          <header class="practice-area-hero__header">
            <h1 class="practice-area-hero__title"><?php echo esc_html( $post_title ); ?></h1>
          </header>

          <div class="practice-area-hero__meta">
            <div class="practice-area-hero__share">
              <span class="practice-area-hero__share-label"><?php esc_html_e( 'Share on:', 'mbn-theme' ); ?></span>
              <ul class="practice-area-hero__share-icons" aria-label="<?php esc_attr_e( 'Share on social media', 'mbn-theme' ); ?>">
                <li>
            ...
            <div class="practice-area-hero__actions">
              <button class="practice-area-hero__action-btn practice-area-hero__action-btn--light" aria-label="<?php esc_attr_e( 'Switch to light mode', 'mbn-theme' ); ?>" data-theme-toggle="light" type="button">
              ...
              <button class="practice-area-hero__action-btn practice-area-hero__action-btn--dark" aria-label="<?php esc_attr_e( 'Switch to dark mode', 'mbn-theme' ); ?>" data-theme-toggle="dark" type="button">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <?php if ( has_post_thumbnail() ) : ?>
        <div class="practice-area-featured-image">
          <?php the_post_thumbnail( 'full', array( 'class' => 'practice-area-featured-image__img' ) ); ?>
        </div>
      <?php endif; ?>

      <article class="practice-area-content">
        <div class="practice-area-content__container">

          <div class="practice-area-content__body">
            <?php the_content(); ?>
          </div>

          <div class="practice-area-content__share-bar">
            <span class="practice-area-content__share-label"><?php esc_html_e( 'Share on', 'mbn-theme' ); ?></span>
            <ul class="practice-area-content__share-icons" aria-label="<?php esc_attr_e( 'Share article on social media', 'mbn-theme' ); ?>">
              <li>
                <a href="<?php echo esc_url( $share_links['x'] ); ?>" aria-label="<?php esc_attr_e( 'Share on X (Twitter)', 'mbn-theme' ); ?>" target="_blank" rel="noopener noreferrer">
      ...
      <div class="practice-area-cta-container">
        <aside class="practice-area-cta" aria-label="<?php esc_attr_e( 'Free consultation call to action', 'mbn-theme' ); ?>">
          <div class="practice-area-cta__logo" aria-hidden="true">
            <img src="<?php echo esc_url( get_theme_file_uri( 'assets/images/logo-mark.svg' ) ); ?>" alt="<?php esc_attr_e( 'Hastings & Hastings', 'mbn-theme' ); ?>" />
          </div>
          <h4 class="practice-area-cta__title"><?php esc_html_e( 'Start Your Free Consultation', 'mbn-theme' ); ?></h4>
          <p class="practice-area-cta__text"><?php echo wp_kses_post( __( 'If you’re unsure where to start or how to manage these different payment options, talking with an <a href="/phoenix-brain-injury-lawyer/">experienced brain injury attorney</a> can help you determine the best path forward.', 'mbn-theme' ) ); ?></p>
          <div class="practice-area-cta__actions">
            <a href="/contact-us/" class="practice-area-cta__btn"><?php esc_html_e( 'CONTACT US TODAY TO SCHEDULE A FREE CONSULTATION', 'mbn-theme' ); ?></a>
            <p class="practice-area-cta__phone"><span><?php esc_html_e( 'CALL TODAY', 'mbn-theme' ); ?></span> <a href="tel:4806053939">(480) 605-3939</a></p>
          </div>
        </aside>
      </div>

      <?php if ( ! empty( $related_information_groups ) ) : ?>
        <section class="practice-area-related">
          <div class="practice-area-related__container">
            <div class="practice-area-related__divider" aria-hidden="true"></div>

            <h2 class="practice-area-related__heading"><?php esc_html_e( 'Related Information', 'mbn-theme' ); ?></h2>

            <div class="practice-area-related__grid">
              <?php foreach ( $related_information_groups as $group ) : ?>
                <?php
                $group_title = isset( $group['group_title'] ) ? (string) $group['group_title'] : '';
                $group_links = isset( $group['links'] ) && is_array( $group['links'] ) ? $group['links'] : array();
                ?>
                <div class="practice-area-related__column">
                  <?php if ( '' !== $group_title ) : ?>
                    <h4 class="practice-area-related__col-title"><?php echo esc_html( $group_title ); ?></h4>
                  <?php endif; ?>

                  <?php if ( ! empty( $group_links ) ) : ?>
                    <ul class="practice-area-related__links">
                      <?php foreach ( $group_links as $link_row ) : ?>
                        <?php
                        $link_field  = isset( $link_row['link'] ) && is_array( $link_row['link'] ) ? $link_row['link'] : array();
                        $link_url    = isset( $link_field['url'] ) ? (string) $link_field['url'] : '';
                        $link_title  = isset( $link_field['title'] ) ? (string) $link_field['title'] : '';
                        $link_target = isset( $link_field['target'] ) && '' !== (string) $link_field['target'] ? (string) $link_field['target'] : '_self';
                        ?>
                        <?php if ( '' !== $link_title ) : ?>
                          <li>
                            <a href="<?php echo esc_url( '' !== $link_url ? $link_url : '#' ); ?>" target="<?php echo esc_attr( $link_target ); ?>"<?php echo '_blank' === $link_target ? ' rel="noopener noreferrer"' : ''; ?>>
                              <img src="<?php echo esc_url( get_theme_file_uri( 'assets/icons/icon-chevron-right-blue.svg' ) ); ?>" alt="" aria-hidden="true" class="practice-area-related__chevron" />
