/**
 * Shared Tailwind Color Palette
 * 
 * Global color definitions used across blocks for consistent theming.
 * Maps Tailwind utility classes to their hex color values.
 * 
 * @package MBN_Theme
 */

export const TAILWIND_COLORS = [
  { name: 'White', color: '#FFFFFF', class: 'bg-white' },
  { name: 'Primary Yellow', color: '#FFF24C', class: 'bg-primary' },
  { name: 'Secondary Blue', color: '#3480C3', class: 'bg-secondary' },
  { name: 'Light Blue', color: '#F3F7FC', class: 'bg-light-blue' },
  { name: 'Accent Gold', color: '#FDE212', class: 'bg-accent-gold' },
  { name: 'Accent Blue', color: '#286FB7', class: 'bg-accent-blue' },
  { name: 'Gray 50', color: '#f9fafb', class: 'bg-gray-50' },
  { name: 'Gray 100', color: '#f3f4f6', class: 'bg-gray-100' },
  { name: 'Gray 200', color: '#E8EAEC', class: 'bg-gray-200' },
  { name: 'Text Heading', color: '#13263E', class: 'bg-text-heading' },
];
